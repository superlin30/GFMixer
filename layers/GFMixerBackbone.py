...
# Cell
class GFMixerBackbone(nn.Module):
    def __init__(self, c_in: int, context_window: int, target_window: int,
                 ...
                 pe: str = 'zeros', 
                 learn_pe: bool = True, fc_dropout: float = 0., head_dropout=0, padding_patch=None,
                 pretrain_head: bool = False, head_type='flatten', individual=False, revin=True, affine=True,
                 subtract_last=False,
                 verbose: bool = False, use_fat=True, **kwargs):
        super().__init__()
        ...
        self.FAB_backbone = nn.ModuleList([nn.Sequential(
            TST_with_FAB(c_in, patch_num=token, patch_len=self.dim_list[i], max_seq_len=max_seq_len,
                        ...
                        pe=pe, learn_pe=learn_pe, verbose=verbose,use_fat = use_fat, **kwargs),
            nn.Flatten(start_dim=-2),
    ...
    def forward(self, z):  # z: [bs x nvars x seq_len]
        ...
        # === Period-Aware Patch Configuration ===

        for i, period in enumerate(self.period_list):
            # === Period-Aware Patch Segmentation ===
            patch_input = self.pad_layer[i][0](z).reshape(z.shape[0] * z.shape[1], -1, period)
            patch_context = self.conv[i](patch_input).reshape(z.shape[0], z.shape[1], -1)[..., :z.shape[-1]]

            """
            [PDF](https://github.com/Hank0626/PDF).
            """
            # === Period-Aligned Patch Embedding ===
            patch_input = self.pad_layer[i][1](patch_input)                          
            patch_emb = self.embedding[i](patch_input.unsqueeze(-3))                
            patch_emb = rearrange(patch_emb, "(b m) d n -> b m d n", b=z.shape[0])   

            # === Fourier-Aware Attention Block (FAB) ===
            FAB_output = self.FAB_backbone[i](patch_emb)  

            # === Residual Aggregation Path (GAB-style) ===

            res.append(FAB_output + patch_context)           
...
class TST_with_FAB(nn.Module):  # i means channel-independent
    """
    ...
    def __init__(self, c_in, patch_num, patch_len, max_seq_len=1024,
        ...
        # Encoder
        self.encoder = FABEncoder(q_len, d_model, n_heads, d_k=d_k, d_v=d_v, d_ff=d_ff, norm=norm,
...
class FABEncoder(nn.Module):

    """
    GFMixer FAB Encoder (Fourier-Aware Block)

    Applies:
    - Patch-wise linear projection
    - Multi-frequency positional encoding via FourierEmbedding
    - Multihead Attention with spectral modulation

    Input:  [B, N, patch_len, patch_num]
    Output: [B, N, D, patch_num]
    """
    def __init__(self, q_len, d_model, n_heads, d_k=None, d_v=None, d_ff=None,
        ...
        self.layers = nn.ModuleList(
            [TSTEncoderLayer(q_len, d_model, n_heads=n_heads, d_k=d_k, d_v=d_v, d_ff=d_ff, norm=norm,
                             attn_dropout=attn_dropout, dropout=dropout,
                             activation=activation, res_attention=res_attention,use_fat=use_fat,
...
class TSTEncoderLayer(nn.Module):
    def __init__(self, q_len, d_model, n_heads, d_k=None, d_v=None, d_ff=256, store_attn=False,
                 ...
                 pre_norm=False, pos=None,use_fat=True):
        super().__init__()
        ...
        self.self_attn = _MultiheadAttention(d_model, n_heads, d_k, d_v, attn_dropout=attn_dropout,
                                             proj_dropout=dropout, res_attention=res_attention, pos=pos,use_fat=use_fat)
...
class _MultiheadAttention(nn.Module):
    def __init__(self, d_model, n_heads, d_k=None, d_v=None, res_attention=False, attn_dropout=0., proj_dropout=0.,
                 qkv_bias=True, lsa=False, pos=None, pos_emb =None,use_fat=True):
        ...
        if self.use_fat:
            # Set config and attach to args or configs
            FAT_configs = get_FAT_config(
                d_model=d_model,
                n_heads=n_heads,
                fourier_dim=64,
                fourier_learnable=False
            )
            self.FAT_emb = FourierEmbedding(FAT_configs, use_rope_cache=False)
        else:
            self.FAT_emb = None
    ...
    def forward(self, Q: Tensor, K: Optional[Tensor] = None, V: Optional[Tensor] = None, prev: Optional[Tensor] = None,
        ...
        k_s = self.W_K(K).view(bs, -1, self.n_heads, self.d_k).transpose(1, 2)   # [bs, n_heads, k_len, d_k]

        v_s = self.W_V(V).view(bs, -1, self.n_heads, self.d_v).transpose(1, 2)  # v_s    : [bs x n_heads x q_len x d_v]


        # ✨ Apply FAT
        if self.FAT_emb is not None:
            max_len = max(q_s.size(2), k_s.size(2))  # q_len / k_len
            q_p = self.FAT_emb(q_s, all_len=max_len)                     # [bs, n_heads, q_len, d_k]
            k_p = self.FAT_emb(k_s, all_len=max_len).transpose(2, 3)     # [bs, n_heads, d_k, k_len]
        else:
            q_p = q_s                            # [bs, n_heads, q_len, d_k]
            k_p = k_s.transpose(2, 3)            # [bs, n_heads, d_k, k_len]
...
class _ScaledDotProductAttention(nn.Module):
    ...
    def forward(self, q: Tensor, k: Tensor, v: Tensor, prev: Optional[Tensor] = None,
        ...
        attn_scores = torch.matmul(q_p, k_p) * self.scale  # attn_scores : [bs x n_heads x max_q_len x q_len]
        #attn_scores = torch.matmul(q, k) * self.scale  # attn_scores : [bs x n_heads x max_q_len x q_len]
